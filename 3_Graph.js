...
class Graph {
    constructor(nodeA, isDir){
        this.matrix = [[0]];
        this.nodos = [];
        this.nodos.push(nodeA);
        if(typeof(isDir) == "boolean")this.directed = isDir;
    ...
    addEdge(v1,v2){
        this.matrix[v1][v2]++;
        if(this.directed == false)this.matrix[v2][v1]++;
    }
    printGraph(){
    ...
    isDirected(){
        for(let x1 = 0; x1 < this.nodos.length; x1++){
            for(let x2 = 0; x2 < this.nodos.length; x2++){
                if(this.matrix[x1][x2] != this.matrix[x2][x1]) return true;
            }
        } return false;
    }
}
